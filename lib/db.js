...
module.exports = class DB {
  ...
  async updateExpirationDate(sid, key, expirationDate) {
    debug(`Update expiration date ${sid}++${key}`);
    const data = this.get(sid).find(item => item.key === key);
    if(!data) return;
    data.metadata.retention = (Number(expirationDate) - Number(data.metadata.createdAt)) / 1000;
    await this.store.update(`${sid}++${key}`, data);
  }
