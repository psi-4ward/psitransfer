...
// Update expiration date of a file
app.post(`${ config.baseUrl }admin/update_expiration_date`, async (req, res, next) => {
  const sid = req.body.sid;
  const key = req.body.key
  const expirationDate = req.body.expirationDate;
  if (sid && key && expirationDate) {
    await db.updateExpirationDate(sid, key, expirationDate);
  }
});
