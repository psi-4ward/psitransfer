...
app.get(`${ config.baseUrl }:sid`, (req, res, next) => {
  if (req.url.endsWith('.json')) {
    ...
    const bcryptFullRegex = /^\$2[ayb]\$12\$[./A-Za-z0-9]{22}[./A-Za-z0-9]{31}$/;
    if(items.some(item => {
      if (!item.metadata.password) return false;
      const pass = downloadPassword || ''; 
      return bcryptFullRegex.test(item.metadata.password) 
        ? !bcrypt.compareSync(pass, item.metadata.password) 
        : item.metadata.password !== pass;
    })) {
...
app.get(`${ config.baseUrl }files/:fid`, async (req, res, next) => {
  ...
  try {
    ...
    res.download(store.getFilename(req.params.fid), utils.toSafeBasename(info.metadata.name, info.key));
  ...
  async function(req, res, next) {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        meta.password = bcrypt.hashSync(meta.password, 12);
