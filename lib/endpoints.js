...
var transporter;
if (config.mailOnUsage.transporter) {
  console.log("mailOnUsages activated");
...
app.get('/keycloak', (req, res) => {
  if (config.keycloak.front) res.sendFile(path.join(__dirname, '../public/assets/keycloak.js'));
  else res.end();
});
...
app.get('/silent-check-sso.html', (req, res) => {
  res.sendFile(path.join(__dirname, '../public/html/silent-check-sso.html'));
});
